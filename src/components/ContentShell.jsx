...
export default function ContentShell({ sidebar, topbar, children, mobileNavOpen, onCloseMobileNav }) {
  return (
    <div className="grid grid-rows-[auto,1fr] md:grid-rows-1 md:grid-cols-[280px,1fr] h-dvh">
      {/* Sidebar - desktop */}
      <aside className="hidden md:flex md:flex-col bg-zinc-900 border-r border-zinc-800">
        <ScrollArea.Root className="h-dvh w-[280px]">
      ...
      <div className={`fixed inset-0 z-40 md:hidden ${mobileNavOpen ? 'pointer-events-auto' : 'pointer-events-none'}`}>
        ...
        />
        <div
          className={`absolute inset-y-0 left-0 w-[86%] max-w-[320px] bg-zinc-900 border-r border-zinc-800 transition-transform ${mobileNavOpen ? 'translate-x-0' : '-translate-x-full'}`}
        >
